...
from utils_dump import bytewidth, feature_to_unsigned_int, threshold_to_unsigned_int, children_dtype
...
def dump_DT_Arr_Baseline_L1(model, file, n_classes, dataset, in_dtype, f_dtype, thr_dtype):
	kernel_c = file[0]
	kernel_h = file[1]
	n = len(dataset)
	...
	for k in range(0, n):
		...
		child_dtype,__,child_bytewidth = children_dtype(__model)
		treeMem, pointerMem, modelMem = model_memory(__model, bytewidth(f_dtype[k]), bytewidth(thr_dtype[k]), child_bytewidth)
		...
		print("#define WHILE_CONDITION (-2)",	file = kernel_h)
...
def dump_DT_Arr_Baseline_Quantized_L1(model, file, n_classes, dataset, in_dtype):
	kernel_c = file[0]
	kernel_h = file[1]
	n = len(dataset)

	# Dump "h" header file
	print("#ifndef __DT_ARR_BASELINE_QUANTIZED_H__", 	 file = kernel_h)
	print("#define __DT_ARR_BASELINE_QUANTIZED_H__\n\n", 	 file = kernel_h)
	...
	for k in range(0, n):
		...
		child_dtype,__,child_bytewidth = children_dtype(__model)
		thr, thr_dtype, thr_bytewidth,__ = threshold_to_unsigned_int(__model)
		f,f_dtype,f_bytewidth,__,__ = feature_to_unsigned_int(__model)
	...
	print("#include \"pmsis.h\"",  file = kernel_c)
	print("#include \"params.h\"", file = kernel_c)
	print("#include \"dt-arr-baseline-quantized.h\"\n\n", file = kernel_c)
...
def dump_DT_Arr_Baseline_L2(model, file, n_classes, dataset, in_dtype, f_dtype, thr_dtype):
	kernel_c = file[0]
	kernel_h = file[1]
	n = len(dataset)
	...
	for k in range(0,n):
		...
		for j in range(0,n_estimators):
			print("	(uint32_t) &randomforest.features_tree%d[0], (uint32_t) &randomforest.threshold_tree%d[0], (uint32_t) &randomforest.children_left_tree%d[0],"%(j,j,j), file = kernel_c)
		print("};\n\n",file = kernel_c)	
...
def dump_DT_Arr_Baseline_Quantized_L2(model, file, n_classes, dataset, in_dtype):
	kernel_c = file[0]
	kernel_h = file[1]
	n = len(dataset)
...
def dump_DT_Arr_Shiftless_L1(model, n_classes, dataset, f, __in, file):
	...
	print("#include \"pmsis.h\"", file = kernel_c)
	print("#include \"params.h\"", file = kernel_c)
	print("#include \"dt-arr-shiftless.h\"\n\n", file = kernel_c)
	
	# Dump header code
	print("#ifndef __DT_ARR_SHIFTLESS_H__", 	 file = kernel_h)
	print("#define __DT_ARR_SHIFTLESS_H__\n\n", 	 file = kernel_h)
	...
	for k in range(0,n):
		...
		treeMem, pointerMem, modelMem = model_memory(__model, f_bytewidth[k], thr_bytewidth[k], child_bytewidth[k])
		...
		print("#define WHILE_CONDITION (%d)"%(-2*f_const[k]),	file = kernel_h)
		print("#define CHILDREN_BYTES (%d)"%(child_bytewidth[k]),	file = kernel_h)
		print("#define NODE2FEAT_DTYPE (%d) /* %s shift */"%(f2node[k]/2,f2node_op[k]),	file = kernel_h)
...
def dump_DT_Arr_Stallfree_L1(model, n_classes, dataset, f, __in, file):
	n = len(dataset)
	f_bytewidth = f[0]
	f_dtype = f[1]
	in_bytewidth = __in[0]
	...
	print("#include \"pmsis.h\"", file = kernel_c)
	print("#include \"params.h\"", file = kernel_c)
	print("#include \"dt-arr-stallfree.h\"\n\n", file = kernel_c)
	
	# Dump header code
	print("#ifndef __DT_ARR_STALLFREE_H__", 	 file = kernel_h)
	print("#define __DT_ARR_STALLFREE_H__\n\n", 	 file = kernel_h)
	...
	for k in range(0,n):
		...
		print("#define CHILDREN_DATATYPE %s"%child_dtype[k],	file = kernel_h)
		print("#define CHILDREN_BYTES (%d)"%(child_bytewidth[k]),	file = kernel_h)
		print("#define CMP_SHIFT (%d)"%(child_bytewidth[k]*2),	file = kernel_h)
		print("#define BIGNODE_DATATYPE %s"%uint_dtype[uint_dtype.index(child_dtype[k]) + 1],	file = kernel_h)
		print("#define NODE2FEAT_DTYPE (%d) /* %s shift */"%(f2node[k]/2,f2node_op[k]),	file = kernel_h)
		print("#define WHILE_CONDITION (%d)"%(-2*f_const[k]),	file = kernel_h)
		#print("\n#define FEAT2INPUT (%d) /* %s Op */"%(f2in[k],f2in_op[k]),	file = kernel_h)
		#print("#define CHILD2NODE (%d) /* %s Op */"%(child2node[k],child2node_op[k]),	file = kernel_h)
...
def dump_DT_Arr_Stallfree_L2(model, n_classes, dataset, f, __in, file):
	n = len(dataset)
	f_bytewidth = f[0]
	f_dtype = f[1]
	in_bytewidth = __in[0]
