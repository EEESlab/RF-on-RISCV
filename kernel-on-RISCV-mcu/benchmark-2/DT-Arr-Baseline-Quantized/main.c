...
#include "params.h"
#include "data.h"
#include "dt-arr-baseline-quantized.h"
...
void cluster_rf(void *arg)
{
    ...
    uint8_t *__features = (uint8_t *)(modelBuffer);
    uint16_t *__threshold = (uint16_t *)(__features + treeDim[transferID*DMA_TREES]);
    ...
    {    
        __features = (uint8_t *)(__children_right + treeDim[transferID*DMA_TREES + i - 1]);
        __threshold = (uint16_t *)(__features + treeDim[transferID*DMA_TREES + i]);
