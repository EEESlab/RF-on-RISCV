...
PI_CL_L1 uint32_t max_nodes[N_SOL] = {249, 496, 729, 958, 1410, 1852, 2728, 3592, 5288, 10096};
PI_CL_L1 uint32_t dma_trees[N_SOL] = {1, 2, 3, 4, 6, 8, 12, 16, 24, 48};
...
void cluster_rf(void *arg)
{
    ...
    int8_t *__features = (int8_t *)(modelBuffer);
    float *__threshold = (float *)(__features + treeDim[transferID*DMA_TREES]);
    uint16_t *__children = (uint16_t *)(__threshold + treeDim[transferID*DMA_TREES]);
    score[dt_arr_stallfree(arg , __features, __threshold, __children)]++;

    for (int i = 1; i < DMA_TREES; ++i)
    {    
        __features = (int8_t *)(__children + 2 * treeDim[transferID*DMA_TREES + i - 1]);
        __threshold = (float *)(__features + treeDim[transferID*DMA_TREES + i]);
        __children = (uint16_t *)(__threshold + treeDim[transferID*DMA_TREES + i]);    
        score[dt_arr_stallfree(arg, __features, __threshold, __children)]++;
    }
...
void cluster_rf_l2(void *arg)
{
    ...
    int8_t *__features = (int8_t *)(paramAddr[core_id*6*3]);
    float *__threshold = (float *)(__features + treeDim[core_id*6]);
    uint16_t *__children = (uint16_t *)(__threshold + treeDim[core_id*6]);
    score[dt_arr_stallfree(arg , __features, __threshold, __children)]++;

    for (int i = (core_id*6 + 1); i < (core_id + 1)*(N_TREES/N_CORES); ++i)
    {    
        __features = (int8_t *)(__children + 2 * treeDim[i - 1]);
        __threshold = (float *)(__features + treeDim[i]);
        __children = (uint16_t *)(__threshold + treeDim[i]);    
        score[dt_arr_stallfree(arg, __features, __threshold, __children)]++;
...
void cluster_delegate(void *arg)
{
    ...
    //     pi_cl_team_fork(N_CORES, cluster_rf_l2, (float *) (x_test + i*DIM));
...
void fabric_controller(void)
{
    ...
    dmaModelBuffer1 = (uint8_t *) pi_cl_l1_malloc(&cluster_dev, (uint32_t) (9 * DMA_NODES_MAX));
    dmaModelBuffer2 = (uint8_t *) pi_cl_l1_malloc(&cluster_dev, (uint32_t) (9 * DMA_NODES_MAX));

    modelBufferArr[0] = dmaModelBuffer1;
    modelBufferArr[1] = dmaModelBuffer2;

    for (int i = 0; i < (9 * DMA_NODES_MAX); i++)
    {
        dmaModelBuffer1[i] = 0;
        dmaModelBuffer2[i] = 0;
    }
